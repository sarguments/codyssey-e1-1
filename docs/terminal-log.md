...
- `ls`는 디렉터리의 항목을 보여 준다.
- `-a`는 이름이 `.`으로 시작하는 숨김 항목까지 표시한다.
- `-l`은 권한, 소유자, 크기, 수정 시간 등을 자세히 표시한다.
- `ls -al workspace`처럼 옵션을 함께 사용하고 확인할 경로를 뒤에 적을 수 있다.
- `ls -l <파일>`은 파일 정보를, `ls -ld <디렉터리>`는 디렉터리 내부 목록이 아닌 디렉터리 자체의 정보를 표시한다.
